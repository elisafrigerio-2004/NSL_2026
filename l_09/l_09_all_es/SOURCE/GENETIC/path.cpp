...
void Path::Crossover(Path & parent1, Path & parent2,Random *rnd){
    ...
    if(parent1._globe!=parent2._globe){ //devono essere costruiti sulla stessa mappa, altrimenti non ha senso il crossover
        cout << "Warning: parent1 and parent2 built on different maps. Aborting." <<endl; 
        return; 
    }
    ...
    if(parent1._globe==nullptr or parent2._globe==nullptr){
        cout << "Warning: at least one parent has no map associated to it. Unable to calculate path lenghts." <<endl; 
        return; 
    }
