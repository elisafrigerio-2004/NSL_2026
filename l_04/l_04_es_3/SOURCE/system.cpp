...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      if (!(_restart)){ //SOLO ES 4.3 (SETUP 4.2)
        _particle(i).setposition(0, this->pbc(_halfside(0)*x, 0)); //SOLO ES 4.2
        _particle(i).setposition(1, this->pbc(_halfside(1)*y, 1));//SOLO ES 4.2
        _particle(i).setposition(2, this->pbc(_halfside(2)*z, 2));//SOLO ES 4.2
      }else{
...
void System :: measure(){ // Measure properties
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy or _measure_temp or _measure_pressure or (_measure_tenergy && _sim_type < 2)){
