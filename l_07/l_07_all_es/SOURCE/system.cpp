...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      break;
    } else cerr << "PROBLEM: unknown input " << property << endl;
  }
  input.close();
...
void System :: initialize_velocities(){
  ...
  } else { // TO BE TEMPORARILY CHANGED IN EXERCISE 4.2-4.3
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open()){
   ...
   if(_measure_cv and !_measure_tenergy){
      cout << "PROBLEM: Unable to calculate specific heat without total energy" <<endl; 
   }
...
void System :: measure(){ // Measure properties
  ...
  double virial=0.0;
  int bin_index; 
  if (_measure_penergy or _measure_pressure or _measure_gofr or (_measure_tenergy and _sim_type <2)) {
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy or (_measure_tenergy  and _sim_type <2)){
  ...
  if (_measure_tenergy){
    if (_sim_type < 2) tenergy_temp = _npart*(kenergy_temp + penergy_temp);
    else { 
      double s_i, s_j;
      for (int i=0; i<_npart; i++){
  ...
  if (_measure_chi){//valido solo per h=0
    _measurement(_index_chi)=magnetization*magnetization; //moltiplicazione per beta verrà fatta al termine del blocco
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
    if(_measure_cv && _measure_tenergy){ // il calore specifico necessita dell'energia totale 
...
void System :: averages(int blk){
  ...
  _average     = _block_av / double(_nsteps);
    ...
    if(_measure_cv && _measure_tenergy){//completa il calcolo di cv (ha bisogno che _measure_tenergy sia true)
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
    ...
    for(int n=0; n<_n_bins;n++){
      norm=cost*(pow(double(n+1),3)-pow(double(n),3));//rho*n_part*4/3*pi*((r+dr)^3-r^3)
